...
.badge-transit  { background: rgba(255,173,31,0.15);  color: #ffad1f; }
.badge-livre    { background: rgba(0,196,167,0.15);   color: #00c4a7; }
.badge-incident { background: rgba(255,80,80,0.15);   color: #ff5050; }
.badge-prep     { background: rgba(120,140,200,0.15); color: #8fa3c0; }
...
with st.expander("⚙️ Paramètres du modèle d'optimisation MIT SC0x", expanded=True):
    ...
    with col3:
        ...
        conso = st.number_input("Consommation (L / 100 km)", min_value=1.0, max_value=100.0, value=12.0, step=0.5)
...
k4.metric("👷 Emplois finançables",  f"{int(economie_fcfa // 756000)}" if economie_fcfa > 0 else "0",
          help="Basé sur un salaire annuel de 756 000 FCFA")

if economie_fcfa > 0:
    st.success(f"✅ L'économie de **{economie_fcfa:,.0f} FCFA/an** peut financer **{int(economie_fcfa // 756000)} poste(s)** de travail local.")
...
# ═══════════════════════════════════════════════
st.markdown('<p class="section-title">🏢 Gestion du Docking — GDIZ</p><div class="section-bar"></div>', unsafe_allow_html=True)
...
with col_left:
    ...
    if submitted:
        if not camion_immat.strip():
            ...
            st.session_state.quais.loc[mask, "Camion"]    = camion_immat.strip().upper()
            st.session_state.quais.loc[mask, "Opération"] = flux_op
            st.session_state.docking_log.append(
                f"✔ {camion_immat.strip().upper()} · {flux_op} · Quai {quai_sel}"
            )
            st.success(f"Accès autorisé : **{camion_immat.strip().upper()}** → Quai **{quai_sel}**")

    # Libérer un quai
    st.markdown("**Libérer un quai**")
    quai_lib = st.selectbox("Quai à libérer", st.session_state.quais["Quai"].tolist(), key="lib")
...
with col_right:
    ...
    # Affichage HTML stylisé des quais
    rows_html = ""
    ...
    if st.session_state.docking_log:
        ...
        for log in reversed(st.session_state.docking_log[-5:]):
            st.markdown(f"<div style='font-size:12px;color:#7a92b0;padding:4px 0;border-bottom:1px solid rgba(255,255,255,0.05);'>{log}</div>", unsafe_allow_html=True)
...
with col_info:
    # Dernière expédition active (En Transit)
    ...
    if not df_transit.empty:
        ...
        st.markdown(f"""
        <div class="card">
            <div style="font-size:11px;color:#5a7090;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:8px;">Dernière expédition en transit</div>
            <div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:700;color:#fff;margin-bottom:4px;">{derniere['ID']}</div>
            <div style="font-size:13px;color:#8fa3c0;margin-bottom:16px;">{derniere['Article']}</div>
            <div style="font-size:13px;color:#c8d8ec;">
                📍 <strong>{position_actuelle}</strong> &nbsp;·&nbsp; {km_faits} km / {DISTANCE_MAX} km
            </div>
        </div>
        """, unsafe_allow_html=True)
...
<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
    <span style="font-size:12px;color:#3d5570;">© 2025 <span style="color:#ffad1f;font-weight:500;">West Africa Logistics Hub</span> · TMS Module</span>
    <span style="font-size:12px;color:#3d5570;">Propulsé par <span style="color:#ffad1f;font-weight:500;">Streamlit</span> · Référentiel <span style="color:#ffad1f;font-weight:500;">MIT CTL</span></span>
</div>
""", unsafe_allow_html=True)
